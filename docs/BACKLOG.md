...
- [ ] **WP-CLI: dump-schema currently exists. Add `wp pediment-ai clear-chat-history` for testing.** Useful in dev to reset the conversation store without DROP TABLE.
...
- [ ] Per-flow `pediment_ai_system_prompt` filter (mentioned as v0.2 plan in `docs/prompts.md`, never implemented).
