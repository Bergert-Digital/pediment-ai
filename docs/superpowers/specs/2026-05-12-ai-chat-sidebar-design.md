...
- New `PluginSidebar` registered as `starter-ai/chat` via `@wordpress/editor` (with `@wordpress/edit-post` fallback for WP <6.6 — mirroring the pattern in `editor/DocumentPanel.tsx`).
- Custom icon in the editor header. Pinnable. Default-closed; auto-opens once on first activation so users discover it.
- The existing `PluginDocumentSettingPanel` shrinks to a single "Open AI chat" button that calls `dispatch('core/editor').openGeneralSidebar('starter-ai/chat')`. Provides a familiar entry point during transition. Removable later.
...
1. Client `POST /wp-json/starter-ai/v1/chat/turns` with `{ conversation_id, post_id, message, selected_block? }`. Server:
...
```sql
CREATE TABLE {prefix}starter_ai_chat_conversations (
...
CREATE TABLE {prefix}starter_ai_chat_messages (
  id                BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
All under `/wp-json/starter-ai/v1/`. One controller: `src/Rest/ChatController.php`.
